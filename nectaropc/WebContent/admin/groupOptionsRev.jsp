...
ResultSet rs2 = stmt2.executeQuery("SELECT * FROM GroupMaster where GroupId='"+rightname+"'");
			while(rs2.next()){
				%>
...
ResultSet rs =stmt.executeQuery("select * from MenuRights where GroupID='"+rightname+"'");
		while(rs.next()){
		groupid=rs.getString("MenuID");


rs2 = stmt2.executeQuery("SELECT * FROM MenuMaster WHERE MenuID=\'"+groupid+"\'");
...
ResultSet rs1 =stmt1.executeQuery("select * from MenuMaster");
while(rs1.next()){
 ...
 <form action="/zalm/admin/groupThrow.jsp" method="post">
 <tr bgcolor="#fff999">

        <td><%out.print(MenuName);%></td>
        <td><input type="submit" name="Submit" value="Add"></td>
        <input type="hidden" name="MenuID" value="<%out.print(MenuID);%>">
