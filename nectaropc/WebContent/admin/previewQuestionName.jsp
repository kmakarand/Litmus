...
<tr valign=top>
		 ...
		 if(request.getParameter("initial")!=null){
			...
			rs = stmt.executeQuery("SELECT * FROM QuestionMaster where ExamID="+ExamId+" ORDER BY QuestionID");
			while(rs.next()){
				 nextval++;
				...
				<td>
<a href='../admin/previewProceed.jsp?questionumber=<%=questionNnumber%>&ExamId=<%=ExamId%>'><%=ques%></a>
					<hr noshade>
				</td></tr>
				<%
					}//start ends
			  ...
			  if(initial > 1)	  {
			out.print("<br><a href='"+request.getRequestURI()+"?initial="+(initial-1)+"&ExamId="+ExamId+"'> << Previous 20 Records << </a><br>");
			  }
			if(nextval > (initial*20))	  {
			out.print("<br><a href='"+request.getRequestURI()+"?initial="+(initial+1)+"&ExamId="+ExamId+"'> >> Next 20 Records >> </a><br>");
						  }
				}
    catch (Exception ex)   {
