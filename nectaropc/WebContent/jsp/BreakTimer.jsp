...
<%
	...
	out.println("if(top.temp2) top.location.href='BreakTimer.jsp?id=" + request.getParameter("id") +"&qans=" + request.getParameter("qans")+"&questimer="+request.getParameter("questimer")+"&timeleft="+request.getParameter("timeleft")+"'");
	out.println("</script>");
...
%>
	...
	<form name="BreakTimer" action="main2.jsp">
	...
	<%
		...
		//out.println("getting size:"+qstore.size());
			...
			String Update_Cand_Temp="Update "+st+" SET  ansg="+ansgiveni+",timetaken="+ttakeni+" WHERE QuestionID="+vi;
			...
			try
				{
								if((tleft!="")||(tleft!=null)||(!tleft.equals("undefined")))
												{
													stmt.executeUpdate("Update ExamStatus SET TimeLeft="+timeleft+",BreaksTaken="+BreaksTaken+"  WHERE CandidateID="+CandidateID);
												}
