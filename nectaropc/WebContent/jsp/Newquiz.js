...
function displayOption(k) 
{
	...
	{
		//if(NoOfSections>1) write('<html><center><BR><h4>Answer Sheet</h4><body  bgcolor="#fff5e7" topmargin=0 rightmargin=0><form name=ans2 action=NewSectionManager.jsp method=get target="_top"><input type=hidden name=timetaken>');
		//else
		...
		if(k!=1)
		{
			...
			//alert(''+);
			//alert(''+);
			for ( i=1 ; i < startQuestion ; i++ ) write('<table width=100% border=0 cellspacing=0 cellpadding=0 bgColor="#fef9e2"><tr><td align=left width=100%>'+i+'</td></tr></table>');
			for ( i=startQuestion ; i <= top.totalQuestion ; i++ )
						{
							color="#FFFFFF";
							if(exammode==0)
								{
									if ( currentQuestion == i )	color="tan";//"#960317";//"#003399";
									if ( currentQuestion > i )  color="#fef9e2";//"#CCCCCC";
									if ( i > currentQuestion )	color="#fef9e2";
								}
								else color="#FFF5E7"
							write('<table width=100% border=0 cellspacing=0 cellpadding=0 bgColor="'+color+'"><tr><td width=20%><FONT-SIZE: 8px;>'+i+'</FONT></td>');
		...
		{write('<br>&nbsp&nbsp&nbsp&nbsp&nbsp<input type=submit value="Submit">');}
	...
	if(timeleft==0)   
		{
			...
			if(currentQuestion<totalQuestion)	
					{	
						temp2.showval();
						if((exammode==0)&&(isPerQuestionTimer==1)) temp2.document.TestMain.submit();
