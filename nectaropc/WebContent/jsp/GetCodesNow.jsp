...
<%
	...
	if(con!=null)
	{
				...
				if((teststart.equals("start"))||((sws!=null)&&(sws.equals("true"))))
				{
					...
					System.out.println("CurrentCodeCount:"+CurrentCodeCount);
					for(int j=0;j<NoOfCodes;++j)
					{
						int count1 =0;
						System.out.println("CodeID[j]t:"+CodeID[j]);
						if(CurrentCodeCount.containsKey(CodeID[j]))
						count1 = 1;//((Integer)CurrentCodeCount.get(CodeID[j])).intValue();

						System.out.println(" Code count obtainbed:"+count1);
				...
				else
				{
					...
					for(int j=0;j<NoOfCodes;++j)
					{
						...
						if(CurrentCodeCount.containsKey(CodeID[j]))
						count2 = 1;//((Integer)CurrentCodeCount.get(CodeID[j])).intValue();
				...
				if((teststart.equals("start"))||((codechange)&&(CurrentCodeCount.size()>0))||((sws!=null)&&(sws.equals("true"))))
				{
					...
					System.out.println("Reaching here inside loop AllCodes"+AllCodes);
					System.out.println("Reaching here inside loop LevelID"+LevelID);
					System.out.println("Reaching here inside loop ExamID"+ExamID);
					...
					System.out.println("QCodeBank"+QCodeBank);
				}
