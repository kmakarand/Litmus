 ...
 * Slotregisteration entities. Transaction control of the save(), update() and
 * delete() operations must be handled externally by senders of these methods or
 * must be manually added to each of these methods for data to be persisted to
 * the JPA datastore.
 * 
 * @see .Slotregisteration
 * @author MyEclipse Persistence Tools
 */

public class SlotregisterationDAO implements ISlotregisterationDAO {
	// property constants
	 ...
	 * SlotregisterationDAO.save(entity);
	...
	public void save(Slotregisteration entity) {
		EntityManagerHelper.log("saving Slotregisteration instance",
				Level.INFO, null);
		try {
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * SlotregisterationDAO.delete(entity);
	 * EntityManagerHelper.commit();
	 * entity = null;
	 * </pre>
	 * 
	...
	public void delete(Slotregisteration entity) {
		EntityManagerHelper.log("deleting Slotregisteration instance",
				Level.INFO, null);
		try {
			entity = getEntityManager().getReference(Slotregisteration.class,
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * entity = SlotregisterationDAO.update(entity);
	 * EntityManagerHelper.commit();
	 * </pre>
	 * 
	 * @param entity
	...
	public Slotregisteration update(Slotregisteration entity) {
		EntityManagerHelper.log("updating Slotregisteration instance",
				Level.INFO, null);
		try {
			Slotregisteration result = getEntityManager().merge(entity);
	...
	public Slotregisteration findById(Integer id) {
		EntityManagerHelper.log("finding Slotregisteration instance with id: "
				+ id, Level.INFO, null);
		try {
			Slotregisteration instance = getEntityManager().find(
					Slotregisteration.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Slotregisteration> findByProperty(String propertyName,
		...
		EntityManagerHelper.log(
				"finding Slotregisteration instance with property: "
						+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from Slotregisteration model where model."
	...
	public List<Slotregisteration> findByAttended(Object attended,
		...
		return findByProperty(ATTENDED, attended, rowStartIdxAndCount);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<Slotregisteration> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all Slotregisteration instances",
				Level.INFO, null);
		try {
			final String queryString = "select model from Slotregisteration model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
